...
############################## ########### #######################################
####################  Filter data on different metrics ###########################

seurat_all_filtered_pct <- subset(seurat_all, subset = 
                       pct_reads_in_peaks > 50 )
seurat_all_filtered_TSS <- subset(seurat_all, subset = 
                       TSS.enrichment > 2 )
seurat_all_filtered_nucleosome <- subset(seurat_all, subset = 
                       nucleosome_signal < 1.5 )
if (length(unique(seurat_all@meta.data$stage)) > 2){
  seurat_all_filtered_fragments_max <- subset(seurat_all, subset = 
...
cell_counts <- data.frame(unfilt = summary(seurat_all@meta.data$stage),
                          ...
                          TSS = summary(seurat_all_filtered_TSS@meta.data$stage),
                          nucleosome = summary(seurat_all_filtered_nucleosome@meta.data$stage),
                          max_fragments = summary(seurat_all_filtered_fragments_max@meta.data$stage),
                          min_fragments = summary(seurat_all_filtered_fragments_min@meta.data$stage))
...
#################   Pct fragments in peaks    ###################
seurat_all <- seurat_all_filtered_pct
after_plot_path = paste0(plot_path, "pct_frags_peaks/")
...
after_plot_path = paste0(plot_path, "nucleosome_enrichment/")
dir.create(after_plot_path, recursive = T)
...
# png(paste0(after_plot_path, 'remaining_cell_table_no_fragment_count.png'), height = 10, width = 40, units = 'cm', res = 400)
# grid.arrange(top=textGrob("Remaining Cell Count", gp=gpar(fontsize=12, fontface = "bold"), hjust = 0.5, vjust = 3),
#              tableGrob(cell_counts, rows=NULL, theme = ttheme_minimal()))
# graphics.off()
