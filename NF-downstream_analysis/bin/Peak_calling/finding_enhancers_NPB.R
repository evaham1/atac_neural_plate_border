...
# this approach: diff accessible in PPR ss8 clusters (C7+C8), then filter on annotation, 
...
open_peaks <- open_across_stages_test(subsetted_raw_matrix, threshold_type = "min", threshold_HH5 = 1,
...
id_data <- id_data %>% mutate(open_early_filter = ifelse(unique_id %in% open_peaks == TRUE, "T", "F"))

### ALTERNATIVE Step 3: filter out peaks that are only open at HH7 or later

closed_peaks <- open_across_stages_test(subsetted_raw_matrix, threshold_type = "max", threshold_HH5 = 0.01,
                                      threshold_HH6 = 0.01, threshold_HH7 = 10, threshold_ss4 = 10, threshold_ss8 = 10)
print(paste0("length of closed_peaks: ", length(closed_peaks))) # 35

subsetted_matrix <- subset_matrix(normalised_matrix, closed_peaks)

png(paste0(plot_path, 'diff_accessible_annot_filtered_closed_early.png'), height = 20, width = 30, units = 'cm', res = 400)
print(marker_heatmap(subsetted_matrix, pal = pal, clusterCols = FALSE, labelRows = TRUE))
graphics.off()

id_data <- id_data %>% mutate(closed_early_filter = ifelse(unique_id %in% closed_peaks == TRUE, "T", "F"))

### Step 4: export peaks and visualise them
write.table(open_peaks, file = paste0(plot_path, "ss8_PPR_putative_enhancers_open_early.txt"), sep = "")
write.csv(id_data, file = paste0(plot_path, "ss8_PPR_putative_enhancers_table.csv"))

# make genome browser plots for open peaks
plot_path <- paste0(plot_path, "browser_tracks_open_peaks/")
dir.create(plot_path, recursive = T)
...
# this approach: diff accessible in PPR clusters at HH7, ss4 and ss8
...
open_peaks <- open_across_stages_test(subsetted_raw_matrix, threshold_type = "min", threshold_HH5 = 1,
                                      threshold_HH6 = 1, threshold_HH7 = 1, threshold_ss4 = 1, threshold_ss8 = 1)
print(paste0("length of open_peaks: ", length(open_peaks))) # 39
...
closed_peaks <- open_across_stages_test(subsetted_raw_matrix, threshold_type = "max", threshold_HH5 = 0.01,
                                        threshold_HH6 = 0.01, threshold_HH7 = 10, threshold_ss4 = 10, threshold_ss8 = 10)
print(paste0("length of closed_peaks: ", length(closed_peaks))) # 7
...
### Step 4: export peaks and visualise them
write.table(open_peaks, file = paste0(plot_path, "HH7_ss4_ss8_PPR_putative_enhancers_open_early.txt"), sep = "")
...
# this approach: diff accessible in NC ss8 clusters (C1), then filter on annotation, 
# then filter so open at all stages OR only open from HH7 onwards
...
open_peaks <- open_across_stages_test(subsetted_raw_matrix, threshold_type = "min", threshold_HH5 = 1,
                                      threshold_HH6 = 1, threshold_HH7 = 1, threshold_ss4 = 1, threshold_ss8 = 1)
print(paste0("length of open_peaks: ", length(open_peaks))) # 13
...
closed_peaks <- open_across_stages_test(subsetted_raw_matrix, threshold_type = "max", threshold_HH5 = 0.01,
                                        threshold_HH6 = 0.01, threshold_HH7 = 10, threshold_ss4 = 10, threshold_ss8 = 10)
print(paste0("length of closed_peaks: ", length(closed_peaks))) # 30
...
# this approach: diff accessible in NC HH7,ss4,ss8 clusters, then filter on annotation, 
# then filter so open at all stages OR only open from HH7 onwards
...
# this approach: diff accessible in NC HH7,ss4 clusters, then filter on annotation, 
# then filter so open at all stages OR only open from HH7 onwards

plot_path <- "./plots/NC/diff_HH7_ss4"
dir.create(plot_path, recursive = T)
...
se <- getMarkerFeatures(
  ...
  useMatrix = "PeakMatrix", 
  groupBy = "stage_clusters",
  useGroups = c("HH7_C5", "HH7_C6", "ss4_C6"))
se <- add_unique_ids_to_se(se, FullData, matrix_type = "PeakMatrix")
unique_ids <- unique(extract_ids(se, cutOff = "FDR <= 0.01 & Log2FC >= 1", top_n = FALSE))
