...
ht <- GRNHeatmap(df.grn.pos, tf.timepoint = df.tfs.tmp$time_point, km = 1)
png(paste0(temp_plot_path, 'TF_gene_corr_heatmap.png'), height = 30, width = 115, units = 'cm', res = 400)
ht
graphics.off()
...
# add which GM node is part of (GMs calculated on full data)
df.gm <- data.frame(
  node <- GM23,
  GM <- rep("GM23", length(GM23))
)
colnames(df.gm) <- c("node", "FullData_GM")
node_metadata <- merge(node_metadata, df.gm, by = "node", all = TRUE)
