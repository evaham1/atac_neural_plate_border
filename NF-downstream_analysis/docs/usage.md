# nf-core/downstream: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/downstream/usage](https://nf-co.re/downstream/usage)
...
```console
nextflow run nf-core/downstream --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/downstream
```
...
First, go to the [nf-core/downstream releases page](https://github.com/nf-core/downstream/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
